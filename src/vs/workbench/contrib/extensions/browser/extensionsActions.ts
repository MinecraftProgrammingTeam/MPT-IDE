...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		return this.extensionsWorkbenchService.uninstall(this.extension).then(() => {
			alert(localize('uninstallExtensionComplete', "Please reload MPT-IDE to complete the uninstallation of the extension {0}.", this.extension!.displayName));
		});
	}
}
			...
			.then(() => {
				return this.extensionsWorkbenchService.reinstall(extension)
					.then(extension => {
						...
						const message = requireReload ? localize('ReinstallAction.successReload', "Please reload MPT-IDE to complete reinstalling the extension {0}.", extension.identifier.id)
